...
func newDedupe() *dedupe {
	fmt.Println("MADHUKANT_1:newDedupe:")
	return &dedupe{s: new(sync.Once)}
}

func (d *dedupe) do(fn func()) {
	fmt.Println("MADHUKANT_1:do:")
	d.l.Lock()
	s := d.s
	d.l.Unlock()
...
func ClusterPoolFunc(pf ClientFunc) ClusterOpt {
	fmt.Println("MADHUKANT_1:ClusterPoolFunc:")
...
func ClusterSyncEvery(d time.Duration) ClusterOpt {
	fmt.Println("MADHUKANT_1:ClusterSyncEvery:")
	return func(co *clusterOpts) {
		co.syncEvery = d
	}
}
...
func ClusterOnDownDelayActionsBy(d time.Duration) ClusterOpt {
	fmt.Println("MADHUKANT_1:ClusterOnDownDelayActionsBy:")
	return func(co *clusterOpts) {
...
func ClusterWithTrace(ct trace.ClusterTrace) ClusterOpt {
	fmt.Println("MADHUKANT_1:ClusterWithTrace:")
	return func(co *clusterOpts) {
		co.ct = ct
	}
}
...
func NewCluster(clusterAddrs []string, opts ...ClusterOpt) (*Cluster, error) {
	fmt.Println("MADHUKANT_1:NewCluster:")
	c := &Cluster{
	...
	fmt.Println("MADHUKANT_Calling_1:Sync:")
...
func assertKeysSlot(keys []string) error {
	fmt.Println("MADHUKANT_1:assertKeysSlot:")
	var ok bool
	var prevKey string
	var slot uint16
...
func (c *Cluster) rpool(addr string) (Client, error) {
	fmt.Println("MADHUKANT_1:rpool:")
	c.l.RLock()
	defer c.l.RUnlock()
	fmt.Println("MADHUKANT_addr:", addr)
	fmt.Println("MADHUKANT_c.pools:", c.pools)
	if addr == "" {
		for _, p := range c.pools {
			return p, nil
		}
		...
		fmt.Println("MADHUKANT_c.pools[addr]:", c.pools[addr])
		return p, nil
	}
	return nil, nil
}
...
func (c *Cluster) Client(addr string) (Client, error) {
	fmt.Println("MADHUKANT_1:Client:")
...
func (c *Cluster) getTopo(p Client) (ClusterTopo, error) {
	...
	fmt.Printf("MADHUKANT_1:getTopo_tt_3:%+v:\n", tt)
	fmt.Printf("MADHUKANT_1:getTopo_tt_3:%+T:\n", tt)
	fmt.Printf("MADHUKANT_1:getTopo_tt_4:%d:\n", len(tt))
...
func nodeInfoFromNode(node ClusterNode) trace.ClusterNodeInfo {
	fmt.Println("MADHUKANT_1:nodeInfoFromNode:")
...
func (c *Cluster) traceTopoChanged(prevTopo ClusterTopo, newTopo ClusterTopo) {
	fmt.Println("MADHUKANT_1:traceTopoChanged:", c.co.ct.TopoChanged)
	if c.co.ct.TopoChanged != nil {
		...
		fmt.Println("MADHUKANT_2:addedNodes:", addedNodes)
		fmt.Println("MADHUKANT_2:removedNodes:", removedNodes)
		fmt.Println("MADHUKANT_2:changedNodes:", changedNodes)
	}
}
...
func (c *Cluster) sync(p Client) error {
	...
	for addr, p := range c.pools {
		if _, ok := tm[addr]; !ok {
			p.Close()
			fmt.Println("MADHUKANT_Came_To_Delete:", addr)
			delete(c.pools, addr)
		}
	}
...
func (c *Cluster) syncEvery(d time.Duration) {
	...
	go func() {
		...
		for {
			select {
			case <-t.C:
				fmt.Println("MADHUKANT_Calling_2:Sync:")
				if err := c.Sync(); err != nil {
					fmt.Println("MADHUKANT_Calling_Err:Sync:", err)
					c.err(err)
				}
			case <-c.closeCh:
				return
...
func (c *Cluster) addrForKey(key string) string {
	fmt.Println("MADHUKANT_1:addrForKey:")
	s := ClusterSlot([]byte(key))
	fmt.Println("MADHUKANT_1:s:", s)
	c.l.RLock()
	defer c.l.RUnlock()
	for _, t := range c.primTopo {
		fmt.Println("MADHUKANT_1:t.Slots:", t.Slots)
...
func (ac askConn) Encode(m resp.Marshaler) error {
	fmt.Println("MADHUKANT_1:Encode:")
...
func (ac askConn) Decode(um resp.Unmarshaler) error {
	fmt.Println("MADHUKANT_1:Decode:")
...
func (c *Cluster) Do(a Action) error {
	...
	fmt.Println("MADHUKANT_2_Do_a:", a)
	fmt.Println("MADHUKANT_2_Do_addr:", addr)
	fmt.Println("MADHUKANT_2_Do_key:", key)
	fmt.Println("MADHUKANT_2_Do_doAttempts:", doAttempts)
	return c.doInner(a, addr, key, false, doAttempts)
}

func (c *Cluster) getClusterDownSince() int64 {
	fmt.Println("MADHUKANT_1:getClusterDownSince:")
	return atomic.LoadInt64(&c.lastClusterdown)
}

func (c *Cluster) setClusterDown(down bool) (changed bool) {
	fmt.Println("MADHUKANT_1:setClusterDown:")
...
func (c *Cluster) traceRedirected(addr, key string, moved, ask bool, count int, final bool) {
	fmt.Println("MADHUKANT_1:traceRedirected:")
...
func (c *Cluster) doInner(a Action, addr, key string, ask bool, attempts int) error {
	fmt.Println("MADHUKANT_1:doInner:")
	...
	// prompt a Sync
	if moved {
		fmt.Println("MADHUKANT_Calling_3:Sync:")
